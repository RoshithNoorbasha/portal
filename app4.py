...
def _stable_df_hash(df):
    """Fast, content-based fingerprint for a DataFrame used as a Streamlit cache key.

    The previous approach hashed a DataFrame by its row count alone
    (`lambda df: df.shape[0]`). Two *different* filtered slices that happen to have
    the same number of rows collided on that key, so cached filter options / summary
    tables from one filter combination could silently be served for another. This
    hashes the actual row content (fast - pandas' own vectorized hasher) so each
    distinct DataFrame gets its own cache entry, and cache hits are trustworthy.
    """
    try:
...
def audit_log_tab():
    ...
    if intact:
        st.success('<i class="fas fa-check-circle"></i> Log integrity verified (hash chain intact)', unsafe_allow_html=True)
    else:
        st.error('<i class="fas fa-exclamation-triangle"></i> Log integrity check FAILED - entries may have been altered', unsafe_allow_html=True)
...
# ==========================================
# 7. UI - Main Tabs (OPTIMIZED WITH CACHING & SESSION STATE)
# ==========================================

# Cache the inverter column detection
@st.cache_data(ttl=3600)
def get_inverter_column(df):
    """Cache the inverter column detection"""
    df_columns_lower_map = {str(c).strip().lower(): c for c in df.columns}
    for col in INVERTER_ID_COLS:
        if col in df.columns:
...
@st.cache_data(ttl=3600)
def get_pv_columns_cached(df):
    """Cache PV voltage and current columns detection"""
    return get_pv_string_columns(df)

# Cache filter options with hash
@st.cache_data(ttl=3600, hash_funcs={pd.DataFrame: _stable_df_hash})
...
@st.cache_data(ttl=300, hash_funcs={pd.DataFrame: _stable_df_hash})
...
@st.cache_data(ttl=60, hash_funcs={pd.DataFrame: _stable_df_hash})
...
# Remove caching from styled summary to avoid pickle issues
...
def create_pv_string_tab(df):
    """Create the inverter-wise PV string details tab with two sub-tabs"""
    
    # ==========================================
    # HEADER WITH SUBTITLE - Using simple HTML for icons
    # ==========================================
    st.markdown("""
    ...
    pv_voltage_cols, pv_current_cols = get_pv_columns_cached(df)
    if not pv_voltage_cols and not pv_current_cols:
        st.warning("No PV string data columns found in the dataset")
        return

    # ==========================================
    # FILTERS SECTION - batched in a form so changing one dropdown
    # doesn't trigger a full-page recompute; only "Apply Filters" does.
    # ==========================================
    st.caption("Pick your filters, then click **Apply Filters** to refresh the tables below.")
    with st.form("pv_string_filters_form"):
        ...
        with col6:
            ...
            selected_grid = st.selectbox("Filter by Grid", ["All"] + available_grids, key="pv_grid_filter")
        with col7:
            status_options = ["All", "Working (>=70%)", "Fair (50-70%)", "Failed (<50%)", "Critical (<30%)"]
            selected_status = st.selectbox("Filter by Status", status_options, key="pv_status_filter")

        st.form_submit_button("🔍 Apply Filters", use_container_width=True)

    # Display-only toggles stay outside the form so they react instantly
    # (they only pick which already-computed columns to show, no heavy recompute).
    ...
    # Apply filters with caching - store in session state for reuse
    ...
    # ==========================================
    # CREATE TWO TABS INSIDE PV STRING DETAILS
    # ==========================================
    tab1, tab2 = st.tabs([
        '📊 INV-PV Details', 
    ...
    with tab1:
        ...
        st.markdown("---")
        st.markdown("### 📋 Inverter-wise Summary")
        ...
        st.markdown("---")
        st.markdown("### ⚡ Detailed PV String Data")
    ...
    # ==========================================
    with tab2:
        st.markdown("### ⚡ Individual String Details")
        ...
        if selected_inverter_detail:
            ...
            if selected_inverter_detail in inverter_data_df[inverter_col].values:
                ...
                # First pass: collect all working string values to calculate average
                working_values = []
                all_string_data = {}
                ...
                for col, pv_num in pv_current_cols_sorted:
                    if col in inverter_data:
                        ...
                        if pd.notna(value):
                            ...
                            # Collect working values for average calculation
                            if status == "Working":
                                working_values.append(value)

                # Calculate average of working strings
                avg_working = sum(working_values) / len(working_values) if working_values else 0
                # Calculate threshold for low performance (20% below average)
                low_performance_threshold = avg_working * 0.8 if avg_working > 0 else 0

                # Filter strings based on selection and determine performance
                string_data = {}
                ...
                if string_data:
                    # Display string status cards in order
                    cols_per_row = 8
                    ...
                    for i in range(0, len(string_list), cols_per_row):
                        ...
                        for idx, (col, data) in enumerate(string_list[i:i + cols_per_row]):
                            ...
                            # Determine color and additional styling
                            if is_negative:
                                color = "#ef4444"  # Red
                                extra_class = "blinking"
                                status_text = "⚠️ NEGATIVE!"
                                ...
                                color = "#fbbf24"  # Yellow
                                extra_class = ""
                                status_text = "⚠️ Low Perf"
                            else:
                                color = "#10b981"  # Green
                                extra_class = ""
                                status_text = "Working"

                            # PV display name with proper formatting
                    ...
                    # Summary of filtered strings with performance breakdown
                    ...
                    col4.metric("⚠️ Low Performance", low_performance)
                    col5.metric("🚨 Negative", negative_count)

                    # Show average working current
                    if working_values:
                        st.info(
                    ...
                    if low_performance > 0 or negative_count > 0:
                        ...
                        with alert_cols[0]:
                            if low_performance > 0:
                                st.warning(
                                    f"⚠️ **{low_performance}** string(s) are performing below the threshold (20% below average)")
                                # List low performance strings
...
def create_plot_charts(plot_summary):
    ...
    fig_bar = px.bar(
        ...
        title="<i class='fas fa-chart-bar'></i> Plot-wise String Status", 
    ...
    fig_avail = px.bar(
        ...
        title="<i class='fas fa-chart-line'></i> Plot-wise Availability (%)", 
    ...
    fig_donut.update_layout(height=400, title="<i class='fas fa-heartbeat'></i> Overall String Health", 
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(size=12),
    ...
    fig_scatter = px.scatter(
        ...
        color="Availability (%)", text="Plot", title="<i class='fas fa-map-marker-alt'></i> Plot Distribution: Inverters vs Strings",
    ...
    fig_treemap = px.treemap(
        ...
        color_continuous_scale=[[0, "#ef4444"], [0.3, "#f59e0b"], [0.5, "#fbbf24"], [0.7, "#34d399"], [1, "#10b981"]],
        range_color=[0, 100], title="<i class='fas fa-palette'></i> String Distribution by Plot",
...
def display_plot_metrics(plot_summary):
    ...
    for idx, (_, row) in enumerate(plot_summary.iterrows()):
        ...
        with cols[col_idx]:
            ...
            if avail >= 90: 
                status_color, status_icon, status_text = "#10b981", '<i class="fas fa-circle" style="color: #10b981;"></i>', "Excellent"
            elif avail >= 70: 
                status_color, status_icon, status_text = "#34d399", '<i class="fas fa-circle" style="color: #34d399;"></i>', "Good"
            elif avail >= 50: 
                status_color, status_icon, status_text = "#fbbf24", '<i class="fas fa-circle" style="color: #fbbf24;"></i>', "Fair"
            else: 
                status_color, status_icon, status_text = "#ef4444", '<i class="fas fa-circle" style="color: #ef4444;"></i>', "Poor"
            ...
            <div style='background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); border: 2px solid {status_color}; border-radius: 12px; padding: 15px; margin: 5px 0;'>
                ...
                <div style='margin-top: 8px;'>
                    <div style='display: flex; justify-content: space-between;'>
                        <span style='color: #94a3b8; font-size: 12px;'><i class="fas fa-info-circle"></i> Status</span>
                        <span style='color: {status_color}; font-weight: bold; font-size: 14px;'>{status_text}</span>
                    </div>
                    <div style='display: flex; justify-content: space-between; margin-top: 4px;'>
                        <span style='color: #94a3b8; font-size: 12px;'><i class="fas fa-microchip"></i> Inverters</span>
                        <span style='color: #f1f5f9; font-weight: bold;'>{int(row['Total_Inverters']):,}</span>
                    </div>
                    <div style='display: flex; justify-content: space-between;'>
                        <span style='color: #94a3b8; font-size: 12px;'><i class="fas fa-chart-simple"></i> Total Strings</span>
                        <span style='color: #f1f5f9; font-weight: bold;'>{int(row['Total_Active_Strings']):,}</span>
                    </div>
                    <div style='display: flex; justify-content: space-between;'>
                        <span style='color: #94a3b8; font-size: 12px;'><i class="fas fa-check-circle" style="color: #10b981;"></i> Working</span>
                        <span style='color: #10b981; font-weight: bold;'>{int(row['Total_Working_Strings']):,}</span>
                    </div>
                    <div style='display: flex; justify-content: space-between; margin-bottom: 8px;'>
                        <span style='color: #94a3b8; font-size: 12px;'><i class="fas fa-times-circle" style="color: #ef4444;"></i> Failed</span>
                    ...
                    <div style='display: flex; justify-content: space-between; margin-top: 4px;'>
                        <span style='color: #94a3b8; font-size: 11px;'><i class="fas fa-percent"></i> Availability</span>
...
def main_dashboard_tab(df):
    ...
    # Mobile-responsive KPI layout - 3 columns on mobile, 6 on desktop
    # Using columns with different ratios for better mobile display
    ...
    if not plot_summary.empty:
        ...
        # Updated column config without emojis
        ...
        # Mobile-responsive download and insights
        col1, col2 = st.columns(2)
        with col1:
...
def main():
    ...
    if not st.session_state.authenticated:
        st.title("☀️ Solar PV String Analytics")  # Can't use HTML in title
    ...
    <div class="welcome-banner">
        <span style="font-size:1.15rem;"><i class="fas fa-hand-wave"></i> Hi, <b>{full_name}</b>!</span>
        &nbsp;&nbsp;<span class="user-badge-{role}">{ROLE_BADGES.get(role, role)}</span>
    </div>
    """, unsafe_allow_html=True)

    # ---- Sidebar ----
    st.sidebar.title("⚡ PV String Template")  # Can't use HTML in title
    ...
    # Filters are batched inside a form: changing Plot/Block/SACU no longer
    # triggers its own full-page rerun - only clicking "Apply Filters" does.
    # This is what was causing "each filter takes time to fetch": every single
    # dropdown click was re-running the ENTIRE script (dashboard, charts, tables).
    with st.sidebar.form("main_filters_form"):
        plots = ["All"] + sorted_filter_options(df_selected["Plot"])
        selected_plot = st.selectbox("Plot", plots)

        plot_scoped_df = df_selected if selected_plot == "All" else df_selected[df_selected["Plot"] == selected_plot]
        blocks = ["All"] + sorted_filter_options(plot_scoped_df["Block"])
        selected_block = st.selectbox("Block", blocks)

        block_scoped_df = plot_scoped_df if selected_block == "All" else plot_scoped_df[plot_scoped_df["Block"] == selected_block]
        sacus = ["All"] + sorted_filter_options(block_scoped_df["SACU"])
